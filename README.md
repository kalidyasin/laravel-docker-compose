...
2. Under the mysql service in your `compose.yaml` file, add the following lines:
...
The current version of Laravel (11 as of today) uses MailPit as the default application for testing email sending and general SMTP work during local development. Using the provided Docker Hub image, getting an instance set up and ready is simple and straight-forward. The service is included in the `compose.yaml` file, and spins up alongside the webserver and database services.
